...
export class CategoriesGroup1 extends React.Component {
  constructor(props) {
    super(props);
    this.changeFilterValue = this.changeFilterValue.bind(this);
    this.categorySelectHandler = this.categorySelectHandler.bind(this);
    this.newCategoryNameInputHandler =
      this.newCategoryNameInputHandler.bind(this);
    this.newCategoryParentSelectHandler =
      this.newCategoryParentSelectHandler.bind(this);
    this.addCategoryHandler = this.addCategoryHandler.bind(this);
    this.state = {
      filterValue: "",
      addCategoryParams: { parent: 0, name: "" },
    };
  }
  ...
  categorySelectHandler(event, id) {
    ...
    checkedStatus
      ...
      : this.props.diselectCategory(id);
  }
  ...
  addCategoryHandler() {
    ...
    this.props.addCategory(
      this.state.addCategoryParams.name,
      this.state.addCategoryParams.parent
    );
  }
  render() {
    return (
      <SidebarItemsGroup header="Categories" border={"bottom"}>
        ...
        <SidebarItem>
          <CategoriesCheckboxList
            categories={this.categoriesFilter(this.state.filterValue)}
            id={0}
        ...
        >
          <div className="sidebar-item-expandable-row">
            ...
            <input
              type="text"
              id="category-input"
              onBlur={this.newCategoryNameInputHandler}
            ></input>
          </div>
          <div className="sidebar-item-expandable-row">
            ...
            <Select
              ...
              onChange={this.newCategoryParentSelectHandler}
            >
...
function CategoriesGroup() {
  ...
  const [addCategoryParams, setAddCategoryParams] = useState({
    parent: 0,
    name: "",
  });
  const categories = useSelector((state) => state.parse.sidebar.categories);
  const selected = useSelector(
    (state) => state.parse.sidebar.selectedCategories
  );
  const dispatch = useDispatch();

  const categoriesFilter = (categoryName) => {
    return filterWithParent(categoryName, categories) || [];
  };
  ...
  const categorySelectHandler = (event, id) => {
    const checkedStatus = event.target.checked;
  ...
  const newCategoryNameInputHandler = (event) => {
    setAddCategoryParams({ ...addCategoryParams, name: event.target.value });
  };

  const newCategoryParentSelectHandler = (event) => {
    setAddCategoryParams({ ...addCategoryParams, parent: event.target.value });
  };
  ...
  return (
    <>
      ...
      <SidebarItem>
        <CategoriesCheckboxList
          categories={categoriesFilter(filterValue)}
          id={0}
          selected={selected}
