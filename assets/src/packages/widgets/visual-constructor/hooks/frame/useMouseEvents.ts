...
export const useMouseEvents: useMouseEvents.UseMouseEvents = () => {
  const mouseOver:useMouseEvents.MouseOver = (event) => {
  ...
  const mouseOut:useMouseEvents.MouseOut = (event) => {
    const targetElemnt=event.target as HTMLElement;
    if (targetElemnt.tagName === 'svg' || targetElemnt.tagName === 'path') return;
    const {className} = targetElemnt;
